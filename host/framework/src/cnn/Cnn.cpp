...
void CNN::firstEpoch(){
	...
	for(int layerNum = 0; layerNum<sizeOfPoollingLayer; layerNum++){
		...
		p_layer->convolution(1,this->wHeightWidthPairs.at(layerNum),MAX_POOLLING);
	...
	f_layer->convolution(1,this->wHeightWidthPairs.at(0),MAX_POOLLING);
...
void CNN::backward(){
	...
	for(int size=0; size<this->batchSize;size++){
		double y=this->fullyConnectedLayers.at(0)->getAfterFullyConnectedLayer()->getvalues().at(size)[0][0];
		double y1=y_1[size];
...
void CNN::polling(){
	...
	for(int layerNum = 0; layerNum<sizeOfPoollingLayer; layerNum++){
		p_layer->convolution(0,this->wHeightWidthPairs.at(layerNum),MAX_POOLLING);
        p_layer = p_layer->getAfterPoollingLayer();
	}
...
void CNN::fullConnect(){
	...
	p_layer->convolution(0,this->wHeightWidthPairs.at(0),MAX_POOLLING);
}